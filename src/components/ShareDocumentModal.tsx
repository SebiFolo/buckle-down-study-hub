...
interface Friend { id: string; username: string; avatar_url: string | null; level: number; }

export function ShareDocumentModal({ open, onClose, documentId, documentTitle }: {
  open: boolean; onClose: () => void; documentId: string | null; documentTitle?: string;
}) {
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-md">
        ...
        {documentTitle && <p className="text-xs text-muted-foreground -mt-2 truncate">{documentTitle}</p>}
              ...
              {filtered.map((f) => (
                <div key={f.id} className="flex items-center gap-3 py-2">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-muted-foreground">Lvl {f.level} · {titleForLevel(f.level)}</div>
                  </div>
                  <Button size="sm" disabled={sharing === f.id} onClick={() => share(f.id, f.username)}>
                    {sharing === f.id ? <Loader2 className="h-3 w-3 animate-spin" /> : <><Send className="h-3 w-3 mr-1" /> Share</>}
                  </Button>
                </div>
              ))}
            </div>
